...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
  ...
  // -----------------Code for processing live preview frame from Camera1 API-----------------------//we are using this
  @Override
  ...
  private void processImage(
    ...
    if (isMlImageEnabled(graphicOverlay.getContext())) {
      ...
      requestDetectInImage(mlImage, graphicOverlay, bitmap, /* shouldShowFps= */ true, frameStartMs)
          .addOnSuccessListener(executor, results -> processLatestImage(graphicOverlay));  //executor from scopedexecutor
  ...
  private Task<T> setUpListener(
    ...
    return task.addOnSuccessListener(
           ...
           /*   if (!PreferenceUtils.shouldHideDetectionInfo(graphicOverlay.getContext())) {
                graphicOverlay.add(
                    new InferenceInfoGraphic(   //set the above overlay of graphics
                        graphicOverlay,
